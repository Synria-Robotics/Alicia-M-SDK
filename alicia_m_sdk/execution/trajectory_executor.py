...
Supports both PV mode (standard) and MIT position mode (high-frequency playback) for motor-specific control.
"""
...
class _BaseTrajectoryExecutor:
    ...
    def __init__(
        ...
        :param use_mit_mode: If True, use MIT position mode for high-frequency playback (motor-specific)
    ...
    def _execute_trajectory(
        ...
        """
        # MIT position mode: use high-frequency playback
        ...
        # Standard PV mode: use set_robot_state
    ...
    def _execute_mit_position_mode(
        self,
        joint_angles: np.ndarray,
        gripper_values: Optional[np.ndarray] = None,
        playback_hz: float = 200.0
    ) -> Dict[str, Any]:
        """Execute trajectory using MIT position mode (high-frequency playback).
        
        This mode is motor-specific and used for high-frequency trajectory playback (200Hz+).
        MIT position mode sends only position data (2 bytes per motor), making it efficient.
        
        :param joint_angles: Array of joint angles [n_points, n_dof] in radians
        :param gripper_values: Optional array of gripper values [n_points] (0-1000)
        :param playback_hz: Playback frequency in Hz (default: 200Hz)
        :return: Dictionary with execution statistics
        """
        joint_angles = to_numpy(joint_angles)
        n_points = len(joint_angles)
        ...
        for idx in range(n_points):
            ...
            try:
                ...
                self.failed_count += 1
                logger.error(f"[MIT位置模式] 发送点 {idx+1} 失败: {e}")
                if self.failed_count > 10:  # Stop if too many failures
                    beauty_print(f"Too many failures ({self.failed_count}), stopping execution", type="error")
                    break
            
            # Control playback frequency using precise_sleep
            ...
            if self.progress_interval > 0 and (idx + 1) % int(playback_hz) == 0:  # Print once per second
                progress = (idx + 1) / n_points * 100
                elapsed_total = time.time() - self.start_time
                logger.info(f"[MIT位置模式] 进度: {progress:.1f}% ({idx+1}/{n_points}), 已用时: {elapsed_total:.2f}s")
        
        exec_time = time.time() - self.start_time
        logger.info(f"[MIT位置模式] 轨迹执行完成，总用时: {exec_time:.2f}s")
        
        beauty_print(f"MIT position mode execution completed:")
        print(f"  Executed: {self.executed_count}/{n_points} points")
        print(f"  Failed: {self.failed_count}/{n_points} points")
        print(f"  Total time: {exec_time:.4f} s")
        if self.executed_count > 0:
            print(f"  Average time per point: {exec_time/self.executed_count*1000:.4f} ms")
            print(f"  Actual frequency: {self.executed_count/exec_time:.1f} Hz")
        ...
        return {
            ...
            'total': n_points,
            'duration': exec_time,
            'target_duration': n_points / playback_hz if playback_hz > 0 else 0.0
        }
...
class JointTrajectoryExecutor(_BaseTrajectoryExecutor):
    ...
    Supports both PV mode (standard) and MIT position mode (high-frequency, motor-specific).
    """
    ...
    def __init__(
        ...
        :param use_mit_mode: If True, use MIT position mode for high-frequency playback (default: False)
        :param playback_hz: Playback frequency in Hz for MIT position mode (default: 200Hz)
        """
        super().__init__(
            robot=robot,
...
class CartesianTrajectoryExecutor(_BaseTrajectoryExecutor):
    ...
    def execute(
        ...
        :return: Dictionary with execution statistics
        """
        # Check IK success rate if provided (only for PV mode)
        if not self.use_mit_mode and ik_success_rate is not None and ik_success_rate < min_success_rate:
