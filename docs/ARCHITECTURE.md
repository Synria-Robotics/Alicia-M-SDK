...
**决策**：所有可能导致关节突变的操作，必须内置「位置锁定首帧」安全序列。

**危险场景与防护**：

| 操作 | 危险 | 安全序列 |
|------|------|---------|
| 使能 (0x09) | 关节突跳到旧目标位置 | 使能后立即以**当前位置**发送首帧 |
| 失能后重新使能 | 用户已手动移动关节 | 读当前位置 → 使能 → 首帧=当前位置 |
| 切换到 MIT | MIT 首帧 pos_ref 不是当前位置 | 读当前位置 → 切模式 → 首帧 pos_ref=当前位置 |
| 切换到 PV | PV 首帧速度不为零 | 读当前位置 → 切模式 → 首帧 pos=当前位置, vel=0 |
| 恢复力矩 (0x05) | 关节弹回到之前的锁定位置 | 读当前位置 → 以当前位置恢复 kp/kd |

**实现原则**：安全序列由 `JointController` 内部自动执行，用户无需手动处理。
...
def torque_on(self, joints=None):
    """恢复力矩（安全序列，防止突跳）"""
    # 1. 读取当前关节位置
    current_pos = self._device.joint_state.angles
    # 2. 发送 0x05 恢复（固件内部恢复 kp/kd）
    self._send_torque_command(joints, lock=True)
    # 3. 立即以当前位置发送 MIT 帧（防止弹回旧位置）
    self._send_position_latch(current_pos)
```
...
切换到 MIT:
  1. 读取当前关节位置
  2. 发送 0x11, addr=0x0B, 值=0x01 (MIT 模式)
  3. 立即发送首帧: pos=当前位置, vel=0, t=0, kp=默认, kd=默认
     → 防止关节突跳

切换到 PV:
  1. 读取当前关节位置
  2. 发送 0x11, addr=0x0B, 值=0x02 (PV 模式)
  3. 立即发送首帧: pos=当前位置, vel=0
     → 防止关节突跳
...
**所有安全序列由 `JointController` 内部自动执行，用户无需手动处理。**
