...
| **MIT 增益预热** | 运动前读取当前 Kp/Kd，并线性过渡到目标阻抗参数 |
...
MIT 模式第一次发送运动命令前，建议先执行增益预热。SDK 会读取当前 Kp/Kd，
保持当前位置不变，并线性过渡到目标阻抗参数，降低第一帧命令造成的跳变风险。
...
robot.set_robot_state(target_joints=[0, -45, -45, 45, 0, 0], speed=30)
```
